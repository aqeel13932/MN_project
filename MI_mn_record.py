...
def create_datasets(snapshot_dir,memory_size):
    episode_length = args.max_timesteps
    obs_shape = (5,5)
    action_shape = (naction,)
    memory_shape = (memory_size,)
    reward_shape = (1,)
    training_steps_shape = (1,)

    h5_file = h5py.File(os.path.join(snapshot_dir, 'autonomy_dataset.hdf5'), 'w')
    obs_dataset = h5_file.create_dataset('observation', shape=(episode_length,) + obs_shape, dtype='i', maxshape=(None,) + obs_shape)
    action_dataset = h5_file.create_dataset('action', shape=(episode_length,) + action_shape, dtype='f', maxshape=(None,) + action_shape)
    memory_dataset = h5_file.create_dataset('memory', shape=(episode_length,) + memory_shape, dtype='f', maxshape=(None,) + memory_shape)
    reward_dataset = h5_file.create_dataset('reward', shape=(episode_length,) + reward_shape, dtype='f', maxshape=(None,) + reward_shape)
    training_steps_dataset = h5_file.create_dataset('training_steps', shape=(episode_length,) + training_steps_shape, dtype='i', maxshape=(None,) + training_steps_shape)
    return [obs_dataset, action_dataset, memory_dataset, reward_dataset, training_steps_dataset]


def dump_episode(episode, datasets):
    """
    Dumps an episode into a file.

    N = num steps per episode
    S = Nx10x10
    A = Nx4
    R = Nx1
    M = Nx16
    ...
    episode: (S, A, M, R, TS)
    datasets: one dataset per field in the episode
    """
    episode_length = episode[0].shape[0]

    for i in range(len(episode)):
        datasets[i][-episode[i].shape[0]:] = episode[i]
        datasets[i].resize(datasets[i].shape[0] + episode_length, axis=0)
...
TestingCounter=0
def TryModel(model,game,model_number):
    global AIAgent,TestingCounter
    TS = np.zeros((args.max_timesteps,1))
    if model_number!=0:
        TS.fill((model_number-32)*4)
    TestingCounter+=1
    if args.render:
    ...
    all_S = np.zeros((args.max_timesteps,5,5))
    all_rest = np.zeros(rest_size,dtype=np.int8)
    all_R = np.zeros(args.max_timesteps)
    all_A = np.zeros((args.max_timesteps,naction),dtype=np.float)
    if args.render:
    ...
    Start = time()-Start
    all_R = all_R[:,np.newaxis]
    return eaten,Start,[all_S,all_A,lstm_episode_data,all_R,TS]
...
memory_size = 128
datasets = create_datasets('{}/{}'.format(EF,args.train_m),memory_size)
if args.clue:
...
for i in range(1,args.num_eps):
    eps_mod = i*100
    print('loading:{}'.format(eps_mod))
    X = load_model('{}/{}/MOD/model_eps:{}.h5'.format(EF,args.train_m,eps_mod))
    model = Model(inputs=X.inputs,outputs=[X.get_layer(index=9).output,X.get_layer(index=7).output])
    ate,sptime, episode=  TryModel(model,game,eps_mod)
    dump_episode(episode,datasets)
    print('episode:{},ate:{},spent:{} seconds'.format(i,ate,round(sptime,3)))

df= pd.DataFrame(data=data)
df.to_csv('{}/{}/states_autonomy.csv'.format(EF,args.train_m),index=False)
